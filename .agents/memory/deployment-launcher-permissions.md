...
**How to apply:** When an artifact uses a shell script as its production run command, verify it has executable mode (`0755`), LF line endings, and smoke-test the exact root deployment command from the workspace root. Explicitly `unset NODE_PRESERVE_SYMLINKS` before starting Node when using pnpm workspaces. In artifact mode, keep build/run settings in `artifact.toml` and avoid root `pnpm store prune` hooks before bundling.