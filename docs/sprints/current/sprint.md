...
- [x] Backend photo content validation double-guard (ADR-052): 11 coarse checks mirroring the FE in `photo_validation.py`, submit-time 400 hook, noseRatio-only pose check (status: completed)
- [x] My Photos settings tab + authenticated media serving (ADR-053): `GET /api/media/token` signed token + owner-or-admin enforcement in `get_media` behind `MEDIA_AUTH_REQUIRED`; FE `mediaUrl()` wired through resolver layer/AppProvider/CvReportView/ProportionsSection; MyPhotosPanel grouped by date with download (status: completed)
