...
- [x] Replit Autoscale healthcheck: middleware short-circuits non-navigational `GET /` to 200 JSON; `document`/`iframe`/`text/html` always get HTML (status: completed)
...
- [x] Replit Preview: theme hydration-safe init, `data-scroll-behavior="smooth"`, treat iframe as navigation for `/` (status: completed)
