...
8. **Outfit comparison baseline (UI only).** Outfit category generation also produces `outfitBaseline` (white tee, face/hair locked). FE outfit slider BEFORE uses `outfitBaseline` when present; hair/aging sliders keep the original front. Baseline is not the edit source for outfit AFTER variants.
