...
3. **Inline CV phrases with grammatical fallbacks.** Weave face shape / hairline / skin color (`skin.skinTone`) via `_cv_anchors` phrase slots. Hair-style prompts must **not** name CV `hairColor` — lock color/texture from the reference image. Do not use `skin.tone` (evenness) in outfit prompts. Missing or sentinel values (`unknown`, etc.) use neutral phrases (`their face shape`); never emit the literal word `unknown`.
4. **Curated multi-variant banks.** Hair uses curated 5-style banks keyed by **style preference** (`resolve_style_preference`: `genderPreference`, then `growBeard` soft proxy) × CV face-shape class (`Oval`, `Round`, `Square`, `Heart`, `Oblong`) plus a `neutral` bank for missing/unknown values (no silent Oval fallback). Outfit uses 5 curated occasion/register entries with preference-keyed descriptors. Aging uses parametric tiers (`+3`, `+5`, `+10` years). One variant per `generate_image_edit` call remains mandatory.
5. **Gallery size via env.** `AI_VISUALS_HAIR_COUNT`, `AI_VISUALS_OUTFIT_COUNT`, and `AI_VISUALS_AGING_COUNT` (defaults 5 / 5 / 3) cap how many bank entries `generate_visual_variants` emits per category.
6. **Front (BEFORE) only.** Image edits MUST use the assessment front portrait (`assessments/{id}/front.jpg` / `photos.front`). Do not use projected AFTER as the AI visuals source. Pipeline and admin generate paths soft-fail / 400 when front bytes are missing.
7. **Media storage for new generations.** When `assessment_id` is set, successful edits MUST `put_bytes` under `assessments/{id}/ai-visuals/{type}/{styleId}.jpg` (or `outfit-baseline.jpg`) and store `/api/media/…` in `imageSrc` (+ optional `relativePath` / `contentType`). Legacy inline `data:image/…` values in existing JSONB remain readable; no backfill required.
8. **Outfit comparison baseline (temporarily disabled).** White-tee `outfitBaseline` generation is commented out while the outfit UI shows AFTER images only (no before/after compare). Helpers remain in `visual_generation.py` for restore. Hair still compares against the original front; aging uses the front as Current. Legacy `outfitBaseline` JSON may remain on old assessments; FE ignores it. Baseline is never the edit source for outfit AFTER variants.
