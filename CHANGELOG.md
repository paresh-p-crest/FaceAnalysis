...
## [Unreleased]
### Added
- **MiVOLO v2 Joint Age & Gender Prediction** — Replaced InsightFace `buffalo_l` age model in [`backend/age_estimation.py`](file:///C:/Users/JayRabari/Documents/FacialAnalysis/backend/age_estimation.py) with MiVOLO v2 (`iitolstykh/mivolo_v2` via HuggingFace `transformers`). Handled both age and gender (`"masculine"` or `"feminine"`) classification in a single forward pass, providing gender estimation with zero extra latency or compute.
- **Dimorphism Visual Gender Override** — Updated `dimorphism_metrics` in [`backend/cv_report.py`](file:///C:/Users/JayRabari/Documents/FacialAnalysis/backend/cv_report.py) to accept `predicted_gender`. Prioritizes the AI model's predicted visual gender over user questionnaire answers (`genderPreference` / `growBeard`) to clamp display labels and "room for improvement" recommendations to the actual visual presentation.
- **Vendored MiVOLO Wheel for Replit** — Built and vendored `mivolo-0.6.0.dev0-py3-none-any.whl` under `wheels/` and pointed `requirements.txt` to it, ensuring 100% portable installation on Replit Python 3.11 containers without build-isolation errors.
### Changed
- **Overview UI Missing After Image Placeholder** — Updated [`ExecutiveSummary.jsx`](file:///C:/Users/JayRabari/Documents/FacialAnalysis/artifacts/myface/components/report/ExecutiveSummary.jsx) to display a centered "Projected Image Pending" text block when the projected "after" image is not generated, mirroring the PDF layout instead of falling back to the landmark-overlaid original photo.
- **Facial Age Disclaimer & Card Title Update** — Updated the `facialAgeDisclaimer` translation key in `en.json` and `de.json` to state `*The age range shown above is just an estimate` / `*Die oben gezeigte Altersspanne ist nur eine Schätzung` respectively. Modified `"facialAge"` to `"Facial Age Range"` / `"Gesichtsaltersbereich"` to keep dashboard cards and HTML templates consistently titled.
- **Facial Age Bar 5-65 Range, Highlight, & Left-Text Layout** — Hardcoded the age scale axis to exactly `5` to `65` inside [`FacialAgePanel.jsx`](file:///C:/Users/JayRabari/Documents/FacialAnalysis/artifacts/myface/components/report/FacialAgePanel.jsx). Implemented a range-based slider visualization where a mist green (`bg-[#5e9f8b]/20`) bar segment highlights the detected age range (e.g. `25-30`) between two vertical ticks, with separate labels under each tick. Expanded the width of the left-side summary column to `w-[6rem]` and added a top padding offset (`pt-[6px]` / `pt-[5px]`) alongside `items-start` to ensure that the large age range display (e.g. `25-30`) is perfectly centered vertically relative to the slider bar. Removed the duplicate sub-label below the large age value to prevent label repetition.
### Changed
- **MiVOLO v2 Age Estimation Model Swap** — Replaced InsightFace `buffalo_l` age model in [`backend/age_estimation.py`](file:///C:/Users/JayRabari/Documents/FacialAnalysis/backend/age_estimation.py) with MiVOLO v2 (`iitolstykh/mivolo_v2` via HuggingFace `transformers`) while preserving the exact same function signatures, thread-safe lazy loader pattern, error logging, and `[14, 70]` age range clamping.
### Added
...
### Fixed
- **InsightFace age estimation init crash (TypeError: not NoneType)** — `backend/age_estimation.py` passed `root=None` to `insightface.app.FaceAnalysis` when the repo-local `models/buffalo_l` dir was absent, which overrode the library's default `~/.insightface` root and crashed `os.path.expanduser(None)`. Now `root` is only passed when the local model dir exists; otherwise InsightFace falls back to its default root and auto-downloads `buffalo_l`.
- **Proportions section missing / crash on empty ratios** — Fixed a crash in `CvReportView.jsx` and `ProportionsSection.jsx` where the entire Proportions section would fail to render if `cvReport.proportions.ratios` was missing or empty. The UI now resiliently renders the top-half facial thirds overview and safely omits the ratio tabs when feature ratios are unavailable.
...
### Changed
- **InsightFace age estimation & 5-year range display (ADR-049)** — Replaced hardcoded `visualAge = 28` with real InsightFace `genderage` predictions in `backend/age_estimation.py` and `backend/opencv_metrics.py`. Present predictions as 5-year rounded ranges (e.g., `28` → `25-30`) in `FacialAgePanel.jsx` and `ExecutiveSummary.jsx` along with an industry prediction disclaimer ("AI-estimated, not a clinical measurement.").
- **SegFormer-grounded facial thirds & hairline column-scan** — Replaced MediaPipe landmark #10 hairline detection with SegFormer column-scan boundary detection (Savitzky-Golay smoothed) and label centroids in `backend/face_parsing_metrics.py`. Updated `backend/pipeline_stages.py` and `backend/cv_report.py` to calculate, store, and persist SegFormer facial thirds in `cvReport.proportions` and update `cvReport.overall`. Updated `cvReport.js`, `faceCrop.js`, `ProportionsSection.jsx`, and `CvReportView.jsx` to render SegFormer facial thirds and visual overlays, with graceful fallback to MediaPipe landmarks for legacy reports.
- **Anti-Semicolon System Prompt Directive** — Added `NO_SEMICOLONS_RULE` (`"PUNCTUATION: Do not use semicolons (';'). Keep the phrasing natural, simple, and human-like."`) to `_NARRATIVE_STYLE_RULES` and `_ASSISTANT_STYLE_RULES` in `text_ai_service.py`, `_NL_STYLE_RULES` in `narrative_orchestrator.py`, and `NARRATIVE_TRANSLATION_SYSTEM_PROMPT` in `narrative_translation.py`.
