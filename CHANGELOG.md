...
### Fixed
- **DE protocol `Report.featurePrimary.*` MISSING_MESSAGE spam** — `localizedFeaturePrimary` / `localizedSubsectionTitle` probed a non-existent top-level key before `protocolModel.*`, and still called `t()` after `t.has` was false (next-intl logs each miss). Now prefer `protocolModel.*` and skip `t()` when `has` is false.
