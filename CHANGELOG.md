...
### Fixed
- **Replit Reserved VM deployment hanging on 1 vCPU / 4 GB RAM** — Prioritized Next.js startup before Python backend in `start-prod.sh` to bind port 3000 immediately; included `/healthz` in `middleware.js` for zero-latency `200 OK` JSON health responses; unbuffered stdout streams for real-time deployment logging on GCE Reserved VM instances.
### Changed
