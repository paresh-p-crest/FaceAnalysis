...
- **Chin summary sentence-safe clamp (EN+DE)** — Removed hard chin `summary.maxLength: 160` from EN/DE generation schemas to avoid provider mid-token truncation. Added post-generation sentence-safe clamping: target ~160 chars, prefer full sentences, and if the first sentence exceeds 160 chars keep that first full sentence intact.
- **Executive hero feature tiles restored (EN+DE)** — Restored two page-1 hero rows (`Staged protocol` + `Projected potential`) and made PDF row count derive from localized `heroFeatures` entries instead of a hardcoded single row.
- **Section regen preserves treatment phases** — Regenerating a single protocol section (e.g. `feature_chin`) now keeps existing `protocolNarrative.treatmentPhases` instead of dropping it, preventing treatment cards from falling back to placeholder content.
- **Section regen preserves localized/custom protocol payload** — Regenerating a single section now merges rebuilt protocol content with existing `protocolNarrative` so locale blocks (`protocolNarrative.de`) and custom metadata are retained instead of being overwritten.
- **Chin summary PDF first-sentence preserve (DE/EN)** — Added a chin-only summary truncation path in `reportPdf.js` that keeps the first full sentence when standard sentence-fit would otherwise clip with ellipsis; allows a small controlled bottom overflow for this card only.
- **Executive hero left list shows all 3 points** — Restored three localized hero bullets on page 1 (`Measured morphology`, `Projected potential`, `Staged protocol`) by supporting up to three `heroFeatures` rows in PDF rendering.
...
- **Protocol narrative diction (ADR-049)** — English generation is feature-led third person (no “The subject presents/demonstrates/exhibits”); finding → visual significance → supported non-surgical action or no-change → synthesis. Null-path few-shots rewritten in consumer-facing English; technical tokens kept for grounding. German is localization (du-form, client-correction few-shots for nose/jaw/eyes), not “preserve English technical terms.” DE sanitizer keeps Latin-1 umlauts. Exact-phrase glossary plus leftover-English repair; decimal commas in code. Skin intro and split-frame pending label are i18n. QOVES is structure-only, never a treatment source.
### Added
- **Re-enabled User Signup & Auth Swapping UI** — Re-enabled user registration end-to-end (`POST /api/auth/register` backend endpoint, `RegisterRequest` model, `_send_signup_welcome()`, and `register()` client export). Restored two-tab toggle UI ("Sign in" / "Sign up") in `AuthForm.jsx` with dynamic `mode` state, first/last name inputs for signup, and switching side panel/header copy so new test users can create accounts.
### Added
...
### Changed
- **LLM reasoning effort + token budget** — Added `LLM_REASONING_EFFORT` config (default `high`), wired into all three completion functions (`chat_structured_completion`, `chat_json_completion`, `chat_text_completion`) for reasoning-capable models (GPT-5-mini via OpenAI or OpenRouter). Raised `LLM_MAX_OUTPUT_TOKENS` default from 8000 → 12000 to accommodate hidden reasoning tokens.
### Fixed
- **PDF overlap + DE closing Denglisch** — Privacy link no longer sits on `Datenschutzerklärung` (sanitizer keeps trailing spaces for width); protocol page-5 heading follows wrapped lead height; static DE closing uses native du-form copy instead of `the subject` → `du` word-swap.
- **Locale persists across browser restart** — `NEXT_LOCALE` is now a 1-year cookie (`localeCookie.maxAge` + explicit write in `LocaleSwitcher`) instead of a session cookie that reset to German after restart.
- **Hair scale labels locale (PDF)** — Baldness-stage panel labels use i18n (`Need Attention` → `Beobachten`, `Extreme` → `Ausgeprägt`); `Normal` stays `Normal`.
- **PDF text sanitize + summary sentence clip** — All jsPDF string draw/measure paths (`text`, `splitTextToSize`, `getTextWidth`) route through `sanitizePdfText` via one doc wrapper; feature summary cards/bars clip at sentence boundaries (`splitTextAtSentences`) instead of mid-line `slice`.
- **Protocol page 5 DE copy** — Localized BEFORE/AFTER lead paragraphs, 11-features intro, radar/dumbbell axis labels (`Brauen`/`Ohren`), and legend (`Projiziertes Potenzial` / `Kundenwerte`).
- **Understanding-results DE copy** — Client-approved `protocolModel.understandingResults` (konzentrieren sich / individuelle Merkmale / kephalometrisch / kombiniert / Feuchtigkeitsversorgung / ausschließlich … ästhetischen Orientierung).
- **Introduction page DE copy** — Client-approved intro (`kephalometrisch durchgeführt` / `Dadurch werden die Befunde…`), limitations wording, and TOC `Ohren` (was `Ohr`) in `messages/de.json`.
- **Disclaimer/privacy page locale (UI + PDF)** — PDF page 2 now draws `Report.disclaimer` i18n copy instead of hardcoded English; DE uses the client-approved notice/privacy paragraphs (incl. umlauts on `vollständige`); `formatProtocolMonth` uses long localized month (`August 2026`).
- **Feature evaluation table locale (UI + PDF)** — Findings/refs on the overview Merkmalsbewertung table now translate CV labels (`Asymmetrisch`, `Ausgewogen`, `Harmonisch`, `Definiert`), localize `Ratio 1.29` → `Verhältnis 1,29`, and fall back to i18n reference copy instead of `—`.
- **Harmony radar axis locale (UI + PDF)** — Compact radar labels use `radarAxesShort` (`Smo`/`Jaw`/`Skin` → `Glä`/`Kie`/`Haut` in DE) instead of hardcoded English abbreviations.
- **Priority mini-card locale (UI + PDF)** — Dashboard/PDF priority feature cards now localize zone titles (`nav.*`), CV classifiers (`CvReport.labels.*`), metric row labels (`miniCardMetrics.*`), and decimal values (`1,29` in DE).
- **Executive hero copy (DE + PDF)** — Shortened `heroBody` to the client-approved intro sentence and reduced hero bullets to one item: **Gestuftes Protokoll** / **Nicht-chirurgischer Phasenplan** (UI + PDF page-1).
- **Analysis time locale (UI + PDF)** — `formatAnalysisTimeDays` localizes the KPI value (`7 days` / `7 Tage`) in the executive dashboard, protocol preview, and PDF page-1 metrics instead of hardcoded English.
- **PDF German umlauts stripped** — `sanitizePdfText` in `reportPdf.js` now keeps WinAnsi/Latin-1 (`ä ö ü ß`) instead of ASCII-only stripping, and maps capital sharp S (`ẞ` U+1E9E) to `ß` before the Latin-1 filter so it is not deleted.
- **Confirm Dialog Button Layout Alignment** — Resolved a visual layout defect where an invisible spinner slot (`Loader2`) occupied a fixed 16px width and gap on the left of the button text even when not loading. Collapse the width (`0px`), margin (`0px`), and opacity of the stable loader container when inactive, leaving the label perfectly centered ([`ConfirmDialog.jsx`](file:///C:/Users/JayRabari/Documents/FacialAnalysis/artifacts/myface/components/ConfirmDialog.jsx)).
- **Flash of Welcome/Questionnaire on Preparing → Dashboard transition** — `handlePreparingDashboard` was calling `resetAnalysisFlow()` (setting `analysisStep → WELCOME`) *before* `goTo(ROUTES.dashboard)`, causing `AnalysisFlow` to briefly render `<QuestionnaireWelcome>` while still mounted on `/analysis`. Fixed by navigating first and letting the pathname `useEffect` handle the reset after `/analysis` unmounts (`AppProvider.jsx`).

- **Questionnaire auto-resizing text input & wrapping fix** — Standardized all text input fields in `Questionnaire.jsx` (direct text questions, details follow-up fields, motivation, and additional notes) to use `AutoResizeTextarea`. Short text starts as a single line with `py-3 px-0` baseline alignment directly on the underline border; as more text is typed, the box automatically expands vertically so all text wraps and remains 100% visible without horizontal cut-offs or scrollbars.
- **Questionnaire navigation button directional alignment** — Aligned the Back button group (`← | Back`) to the left edge (`justify-start`) of its pill container and the Next button group (`Next | →`) to the right edge (`justify-end`) of its pill container in `Questionnaire.jsx`. Elements remain tightly grouped (`gap-2.5`) without empty interior flex gaps. Passed `backWithChevron` to `AnalysisFlowHeader` so top header retains its left back arrow/chevron icon (`<ChevronLeft /> Back`). Removed duplicate arrow prefix from `Questionnaire.common.back` in `en.json` and `de.json`.
- **Language switcher & header margin consistency** — Applied a subtle `-12px` right margin offset to `.analysis-flow-header__right` in `Questionnaire.css` and normalized panel side padding (`px-8` on PhotoUpload, `px-10` on Questionnaire). The language switcher button now sits at a perfectly balanced ~20px distance from the right panel border across all analysis flow pages (Questionnaire, Photo Upload, Welcome, and Confirmation), avoiding both excessive margins and edge touching.
- **Replit Reserved VM deployment hanging on 1 vCPU / 4 GB RAM** — Converted all `.sh` scripts (`start-prod.sh`, `replit-start.sh`) to Unix LF (`\n`) line endings and created `.gitattributes` (`*.sh text eol=lf`) to resolve Linux `execve` `/bin/bash\r` shebang crashes on Windows checkouts; added `HEAD` HTTP method support to `middleware.js` and `app/healthz/route.js` so `HEAD` health check probes return `200 OK` (preventing 405 Method Not Allowed); added multi-path fallback Next.js binary resolution in `start-prod.sh` to resolve `$WORKSPACE_ROOT/node_modules/next/dist/bin/next` without requiring `pnpm` in global PATH; prioritized Next.js startup before Python backend in `start-prod.sh` to bind port 3000 immediately; unbuffered stdout streams for live deployment console logging.
- **OpenRouter vision narratives (ADR-046)** — `openai_vision_narrative_enabled()` supports OpenRouter allowlisted models (`openai/gpt-5-mini`), sending base64 pose image attachments alongside text prompts when using OpenRouter.
- **Streamlined vision pose mapping** — `FEATURE_VISION_POSES` updated globally so every narrative feature sends exactly 1 pose image (`front` or `smile`), with `ears` as the sole exception receiving 2 images (`front` + `rightProfile`).
### Changed
- **Unmasked Square Ear Photo Crop** — Updated `extract_profile_ear_crop` and `_bbox_from_mask` in `backend/face_parsing.py` to extract 1:1 square crops directly from the unmasked profile photo with minimal padding (`pad=8`), removing white background mask isolation. Updated `EarReportPanel.jsx` to display a 1:1 square frame (`aspect-square object-cover`). Added unit test `test_extract_profile_ear_crop_is_square_unmasked_photo` in `test_face_parsing_crops.py`.
- **Ear Cropout & Naso-Aural Proportion Alignment** — Added SegFormer earrings label 15 to `EAR_LABEL_IDS` (`face_parsing.py`) so AI bounding boxes include jewelry-obstructed earlobes; passed `feature_crops` into `build_profile_report` and `analyze_profile` (`analyze_face.py`, `profile_cephalometrics.py`) to override MediaPipe ear bounds with exact SegFormer ear pixel coordinates; implemented notebook-style caliper overlay rendering in `build_naso_aural_proportion_overlay`; updated `cvReport.js` and `ProportionsSection.jsx` to consume backend naso-aural overlays dynamically for left/right profile views.
- **User-facing landmark count display (170+)** — Updated all user-facing frontend text strings and dashboard KPI constants (`messages/en.json`, `messages/de.json`, `Report.jsx`, `reportProtocolModel.js`, `reportPdf.js`) to display `170+` (curated human-meaningful landmark count) instead of `468+`/`478`. Backend engine files and internal test stubs intentionally preserved at `478`.
- **OpenRouter structured LLM** — `chat_structured_completion` uses strict `json_schema` (with `json_object` fallback) for OpenRouter models `openai/gpt-5-mini` and `google/gemma-4-26b-a4b-it:free`; other OpenRouter/Groq models stay on `json_object` (ADR-045).
...
- **DE narrative translation (ADR-044 Amendment)** — Features, executive content, closing paragraphs, and overview translate using flat-key structured batches (`chat_structured_completion` with `require_strict=True`); Python reassembles DE narrative structure with preserved English subsection titles. Per-field plain-text fallback retained per logical block on key set mismatch or API error. Treatment phase `<<tag>>` packing unchanged.
...
### Fixed
- **PDF treatment protocol whole-bullet fit** — Phase cards drop complete bullets to fit (phase03 keeps ≥1). Mid-word tails from the 280-char detail clamp are trimmed to the last sentence/word for PDF display; backend `_clamp_str` now truncates on sentence/word boundaries. Summary is reserved in the column budget and drawn with word-boundary wrap (no `…`).
- **Visuals enlarge preview** — Removed dark `bg-black/70` backdrop from image enlarge modals (user visuals + admin review). Modal portals to `document.body` for true viewport centering; close button sits just outside the image to the right (`left-full ml-1.5`).
- **Protocol PDF hair/jaw bottom-row alignment** — Hair Health and Further Enhancement titles align with the summary-card title baseline (`cardY + 16`). Summary stays bottom-anchored; left column moves up independently or sentence-truncates at the content-above ceiling when body would overflow `PAGE_BOTTOM`. HTML preview uses `report-pdf-cols--bottom-anchor` for the same bottom pin + title inset.
- **PDF chin page summary overlap** — `drawSummaryBar` no longer pulls the bar above the caller's Y (fixes BEFORE/AFTER overlapping Chin Summary); chin page budgets BEFORE/AFTER height to the remaining page space.
- **PDF privacy link overflow** — Page 2 privacy-policy prefix + URL link now wrap within the right column via `drawWrappedPrivacyLink` (`splitTextToSize` + per-line link annotation); fixes horizontal clipping in DE (and other long locales).
- **Protocol PDF cheeks summary overlap** — `drawSummaryBar` sizes the title column from measured text width (DE labels like "Zusammenfassung Wangenregion" no longer collide with summary body). HTML summary bar title column uses matching `min-w`/`max-w` flex bounds.
- **DE narrative template fallback copy** — German hardcoded templates use localized feature labels (`Wangen` not `midface and cheeks`) and `_facts_phrase` drops camelCase CV keys (`cheekboneHeightClass: High` → `High`) when LLM/translation fallback runs.
- **DE narrative translation token cap** — `narrative_translation.py` now uses `LLM_MAX_OUTPUT_TOKENS` (default 8000) instead of hardcoded 2048, matching EN narrative generation and avoiding empty GPT-5-mini responses when reasoning consumes the smaller budget.
...
- **Admin panel refresh `insertBefore` NotFoundError** — Refresh / delete buttons keep both Lucide icons mounted and toggle opacity (same pattern as protocol edit dock) instead of swapping `Loader2` ↔ `RefreshCw`/`Trash2` mid-commit.
- **Treatment-phase DE pack** — Phase `title`/`duration`/item `name`/`detail` translate in one labeled `<<tag>>` call per phase (plus one for summary): **4** `chat_text` calls instead of up to 25. Reduces free-model empty-response abort risk on dense phase blobs.
- **Protocol PDF/HTML static chrome (no LLM)** — Introduction, Contents, Understanding, feature split titles, subsection display titles, BEFORE/AFTER/PROFILE tags, and PAGE/SEITE header use `Report.protocolModel` / `Pdf` i18n on both HTML protocol preview and jsPDF download (`pdfMessages`). Narrative bodies still use stored `.de` (ADR-044). Admin edits remain keyed on English subsection titles.
- **Protocol edit dock busy icons** — Stable opacity-toggle icon slot + defer `onSaved` via `queueMicrotask` so parent remount no longer triggers `insertBefore` NotFoundError after retry/generate.
- **CV Facial Assessment `explanationDe` at build time (Layer A)** — Parallel German f-string builders in `backend/cv_report_explanations_de.py` emit `explanationDe` beside every `explanation` during CV build (face shape, symmetry, dimorphism, proportions, feature metrics, averageness, hair/smile/eye). Removed LLM post-hoc `cv_report_translation.py` and narratives-stage backfill; legacy assessments need a CV re-run for German ERKLÄRUNG bodies.
- **Report DE remaining gaps** — Extended `CvReport.labels` for eyes, brows, skin, symmetry regions, proportions ratio labels. Proportions expectation/ratio headers use `Report.proportions.ratioTabs.*` i18n. Symmetry explanation header uses `ERKLÄRUNG`. Dimorphism per-feature titles use `Report.nav.*`. Eye/Smile/Neck panels wired for label keys + eye narrative prose.
- **Pipeline retry & recovery** — Stale `running` pipelines reclaimed on worker startup (stage preserved). Admin retry modal: **Resume failed parts** (`mode=resume`, default) vs **Full clear restart** (`mode=full`). Per-stage completion skip on non-force resume; structured pipeline logging (claim/reclaim/stage start/end/skip).
- **Transactional email (Resend)** — Provider-agnostic `send_email()` facade (`backend/email_service.py`) with Resend adapter (production) and SMTP fallback (local dev). Templates: signup welcome, password reset, report ready. Every send logged to `email_send_logs`.
- **Signup welcome email** — Fire-and-forget after `POST /api/auth/register` (registration succeeds even if email fails).
- **Token-based password reset** — `POST /api/auth/forgot-password` (rate-limited, no enumeration) + `POST /api/auth/reset-password` with `{ token, newPassword }`. New `/auth/reset?token=` page; forgot-password modal is email-only.
- **Report-ready email** — Sent when assessment status transitions to `approved`/`published`; link to `/report?assessmentId=…` with server-side ownership check via `GET /api/assessments/{id}`.
- **Email env vars** — `EMAIL_DEFAULT_PROVIDER`, `RESEND_API_KEY`, `EMAIL_FROM_ADDRESS`, `EMAIL_FROM_NAME` in `.env.example`. Replit: connect Resend connector and verify `myface.club` domain for production delivery (sandbox delivers only to verified addresses).
- **Auth UX error messages** — Login error uses a single generic `Auth.loginError` key (no enumeration leak). Forgot-password success copy updated to standard non-leaking wording ("If an account exists for this email, we've sent a password reset link.").
...
### Changed
- **Questionnaire welcome copy** — Description updated to “…Then upload a photo to begin your facial analysis.” (EN + DE).
- **Admin draft reports** — Latest Reports / Review rows with `draft` status hide Open and Delete; show a short note that the user has not fully submitted their analysis yet.
- **Questionnaire UI layout** — Question content and answer inputs in Questionnaire are now vertically centered within the split container (matching the welcome layout) while keeping the top header and bottom navigation buttons anchored.
- **Analysis flow split-panel layout** — Welcome, Questionnaire, and Photo upload restore desktop bottom-anchored content; mobile stays vertically centered. Shared `AnalysisFlowHeader` aligns back/nav and locale switcher on one row; locale overlay remains fixed only on the Preparing step. Photo upload back control uses chevron + sentence-case label; welcome intro and stat cards stay centered above mid-viewport; Get Started pins near the panel bottom (slight lift from edge); welcome header vertically centers Back with the locale pill.
- **Analysis preparing profile photo** — Slightly larger avatar on the preparing timeline; dashboard preparing state resolves the submitted front photo from canonical media URL when list summaries omit `photos`. Preparing content is vertically centered in the viewport below the navbar with more top breathing room.
- **Settings password fields** — New and Confirm new password on Settings → Password now include show/hide eye toggles (same pattern as auth forms).
- **Billing checkout** — Removed Pay with PayPal button from the Billing UI; Stripe checkout remains.
- **Feature Analysis panels** — Removed bottom EXPLANATION prose boxes (`FeatureProseBlock`) from interactive feature tabs; protocol narrative remains on the Protocol section only.
- **Facial Assessment explanation cards** — EXPLANATION blocks on dimorphism, prototypicality, and symmetry use white `report-view-metric-card` background instead of mint `surface-warm`.
- **Privacy policy link** — Disclaimer/PDF privacy link points to [myface.de/datenschutzerklaerung](https://myface.de/datenschutzerklaerung) instead of myface.club.
- **Report close button on protocol PDF** — Close control moves to the sidebar header on the Protocol section so it no longer overlays the embedded PDF viewer; admins always get the sidebar close control.
- **Admin AI visuals in report sidebar** — After report approval, admins see an AI Visuals nav group (hairstyle, outfit, aging) below Protocol in the report sidebar.
- **AI visuals style panel (hair/outfit)** — Right-side suggestion panel uses a soft off-white/gray card background with a border so it reads clearly against the white page; attribute cards stay white with border and shadow for contrast. Tighter horizontal gap between image and panel columns (`gap-x-4`).
- **AI visuals healthy aging header** — Aging tab uses the same split title/subtitle header as hair/outfit; subtitle is portrait-based only (educational disclaimer lives in “Learn more” tooltip).
- **AI visuals about tooltips** — Hairstyle/outfit “About” copy shows in a dark hover tooltip (report-style); section header uses split title + bold subtitle emphasis. — `/ai-visuals` hair and outfit use stacked suggestion rows (title, 2×2 attrs, explanation). Hair keeps before/after slider vs original front; outfit shows AFTER image only. About link on section headers. White-tee `outfitBaseline` generation temporarily commented out (code retained for restore); stage completeness no longer requires baseline.
- **AI visuals preview modal** — Expanded image overlay now sits above the navbar so the close button remains fully visible and clickable.
- **Report opening/close UX** — Opening button stays in "Opening..." state until report content is actually ready (defers `openingReportId` clear until `showReport` is true). Admin Close button moved from mint gap into navbar action cluster. Admin navbar tabs and logo dismiss the open assessment. `AdminPanelPage` uses shared `openingReportId` instead of local state.
- **Password reset API (breaking)** — `POST /api/auth/reset-password` body changed from `{ email, newPassword }` to `{ token, newPassword }`. Admin test email uses `signup_confirmation` template via `send_email()`.
...
### Fixed
- **Hairstyle AI visual framing** — Hair prompts keep the head-and-shoulders crop (`don't zoom out to show full length`). Long-layer style descriptors no longer use ambiguous "long" hair-length wording that pulled the model into full-length shots. — Page-1 treatment protocol cards cap wrapped lines to the right-column budget so long (especially German) phase text no longer draws past the footer; phase summary respects remaining space. DE `treatmentPhases` are char-clamped after translation (same limits as EN generation).
- **Report `CvReportView` import** — Restored missing `CvReportView` import in `Report.jsx` (runtime `ReferenceError` on report open).
- **`/api/health` HEAD 405** — Uptime monitors using `HEAD` got Method Not Allowed because the route was GET-only; now accepts `GET` and `HEAD`.
...
- **Skin analysis (LAB + skin-mask)** — Backend replaces region-box brightness metrics with notebook-aligned MyFace fields (undertone, blemishing, evenness, texture, roughness/homogeneity RIN, oiliness skew, under-eye L*). `SkinReportPanel` shows those fields in the existing summary/carousel/table layout.
...
- **Report sidebar** — Scrollbar hidden on inner `.report-view-nav-scroll` (unlayered CSS overrides global scrollbar styles).
...
### Changed
- **AI Visuals layout** — Category content flush left next to sidebar (override report page `max-w-4xl` centering); tighter canvas padding on tool pages. — Mint background + `report-shell-inner` side gutters and white `report-view-layout` card match the report shell; canvas padding aligned (`p-5 sm:p-7 lg:p-9`).
...
- **Protocol admin HTML preview spike** — admins on unapproved reports see the full `ProtocolReport` HTML document (all pages) in the Protocol tab instead of the PDF iframe, for evaluating in-place edit UX; non-admins still get the PDF iframe. HTML sheets are locked to jsPDF A4 (595×842) and fit-to-width scaled like the PDF iframe.
- **Protocol PDF viewer chrome** — removed in-viewer Protocol title, zoom, fullscreen, and Download PDF controls; iframe now fills the report canvas (`report-view-page--protocol`).
...
### Added
- **Report UI i18n namespace** — Converted report navigation (`labelKey` in `reportNavConfig.js`, translated in `ReportNavSidebar`), intro/disclaimer/executive summary, proportions/dimorphism/averageness, shell chrome (`Report.jsx`), protocol side rail, locked gate, and static paragraphs in `reportProtocolModel.js` to next-intl `Report` namespace in `artifacts/myface/messages/en.json` and `de.json`.
...
- **Orbito-nasal proportion overlay** — replaced corner dots + full-height vertical guides with two horizontal measurement bars (inner canthi + alae) and downward end ticks, matching the MyFace span style. Crop/zoom unchanged.
...
- **AnalysisPreparing UI** — report-style “your analysis is being prepared” screen with live stage list; users can close the tab and track progress from Dashboard/History.
...
- **Proportions overview UI (MyFace layout)** — left face photo with hair/brow/nose/chin dotted guides; right stack of Proportionality score card (High/Good/Fair badge), Facial Thirds C→B→A bar, and Explanation. Backend `proportionLines` now crop-relative to match `imageSrc`.
...
- **Facial Assessments copy (no LLM)** — dimorphism / prototypicality / proportions / symmetry / face shape use second-person report-style metric templates; proportions UI shows facial-thirds overview + per-ratio explanations (ADR-022).
...
- **Protocol web viewer A4 sheets** — in-app protocol pages use `.report-view-a4-page` (210×297 aspect) on a desk canvas so the UI reads like stacked A4 pages, matching the PDF export proportions.
...
- **`reportProtocolModel.js` syntax** — restored missing `(` in `if (strengths.length)` that broke the Next.js build.
...
- `utils/reportProtocolModel.js` fallback copy sanitized (no injectables/lasers/procedures in defaults).
...
- Backend fallback PDF markdown notes MyFace client PDF as canonical when photo available.
### Added
...
### Changed
- Redesigned downloaded PDF report pages 2, 3, and 4 to align with MyFace layout line-by-line, including vertical/horizontal dividers, even text distribution, and MyFace top-left text branding.
...
- `Report.jsx` legacy markdown report tabs and `utils/openai.js` removed; MyFace `cvReport` view is the sole report surface.
### Removed
...
### Added
- MyFace-faithful multi-view CV pipeline (v2): per-pose MediaPipe landmarking (`backend/multi_view.py`), ruler calibration step (`RulerCalibration.jsx`, `mouthWidthMm`/`philtrumLengthMm`), profile cephalometrics (`profile_cephalometrics.py`), quarter oblique analysis (`quarter_analysis.py`), smile dentofacial (`smile_analysis.py`), top-head hair analysis (`hair_analysis.py`).
...
- Orbital proportion tab aligned to MyFace (inter-eye spacing vs eye width); backend now generates ratio overlays and explanations.
- Prototypicality score: fixed inverted middle-third calculation that collapsed scores to ~9; explanations match MyFace tone.
- Dimorphism overview: richer report-style narrative and per-feature explanations.
- Pipeline calibration vs MyFace: symmetry score recalibrated (8 mirror pairs incl. ears; typical faces ~72–82, not inflated 90+); naso-aural ratio measured from profile photo (nasion→subnasale vs visible ear span); front-view naso-aural marked `front_estimate` until profile enrichment.
- Prototypicality score recalibrated for MediaPipe landmarks: jaw width (172↔397), nose/cheek ratio (127↔356), updated cohort norms, softer penalty curve — typical faces now land ~70–80 (target band) instead of collapsing to ~23.
### Changed
...
### Added
- report-style frontend PDF overhaul: full-bleed cover, two-column disclaimer/privacy and closing pages, dumbbell feature comparison chart (replacing radar), per-feature two-column layouts with summary bars, empty AFTER image placeholders, and `generateProtocolNarrative()` for dense LLM copy with named actives/procedures.
- Shared `PRIVACY_PARAGRAPHS` and `LIMITATIONS_PARAGRAPH` in `utils/reportProtocolModel.js`; PDF-quality crops via `cropFeatureBeforeForPdf()` and `cropProfileBefore()` in `utils/aestheticProjection.js`.
### Changed
