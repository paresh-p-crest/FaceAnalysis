...
import { computeAnthropometrics } from './anthropometrics'
...
export function computeMetricsFromLandmarks(landmarks, answers, imageStats = {}) {
  ...
  const anthropometrics = computeAnthropometrics(landmarks, answers)
  const anthHarmony = anthropometrics?.featureScores?.harmony

  const harmonyScore = Math.min(
    99,
    Math.round(
      parseFloat(symmetry) * 0.35 +
      parseFloat(proportionality) * 0.25 +
      (anthHarmony || 75) * 0.25 +
      sharpness * 0.08 +
      brightness * 0.07
    )
  ...
  return {
    ...
    demographics: {
      ...
      ageRange: answers?.ageRange || '25-34',
    },
  }
}
