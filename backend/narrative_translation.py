...
from .narrative_schemas import FEATURE_SUMMARY_MAX, subsection_body_limits
...
_CHAR_LIMIT_GUIDANCE = (
    ...
    "feature summary <=500 (keep full sentences when trimming); subsection body <=2000 (shorter for brief sections); "
...
async def _translate_feature_narrative_llm_plain(narrative: dict, feature_id: str) -> dict:
    ...
    summary_de = _clamp_summary_keep_sentence(summary_de, FEATURE_SUMMARY_MAX)
...
async def _translate_feature_narrative_llm(narrative: dict, feature_id: str) -> dict:
    ...
    if "summary" in en_fields:
        ...
        keys_and_limits["summary"] = FEATURE_SUMMARY_MAX
    for i, s in enumerate(subs_en):
        key = f"subsection_{i}"
        if key in en_fields:
