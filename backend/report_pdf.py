...
def _markdown_to_flowables(markdown: str, styles) -> list:
    ...
    while i < len(lines):
        ...
        if header_match:
            ...
            if level <= 2:
                flowables.append(Paragraph(text, styles['Heading1'] if level == 1 else styles['Heading2']))
            else:
        ...
        if num_match:
            ...
            flowables.append(Paragraph(f"{num}. {text}", styles['Body']))
            i += 1
            continue
...
def generate_pdf_bytes(markdown: str, report_data: Optional[dict] = None) -> bytes:
    ...
    # Title
    flowables.append(Paragraph("AuraScan — Facial Analysis Report", styles['Title']))
